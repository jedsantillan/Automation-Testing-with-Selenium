...
2. Jupiter Toys is expected to grow and expand its offering into books, tech, and modern art. We are expecting the of tests will grow to a very large number.
	...
	a.) Use of headless test execution.
	b.) Segregate test cases such that each group of tests are independent from each other. By doing this, we can implement parallel test execution where each group are assigned to different machine.
